...
@app.post("/is-fraud")
async def is_fraud_post(inp:payment_fraud_check_input, 
    db: Session = Depends(src.db_process.get_db)):
    ...
    db.add(curr_call)
    db.commit()
    step_amnt_nDst_cnt = db.query(func.count(
        src.db_process.apiCalls.id)).filter(
        src.db_process.apiCalls.step==inp.step).filter(
        src.db_process.apiCalls.amount==inp.amount).filter(
        src.db_process.apiCalls.nameDest==inp.nameDest).scalar()
